...
let todostill = data.filter(item => !item.completed);

// console.log(todostill);

// filter for user 6's tasks
let USERSIX = data.filter(item => item.userId == 6);
// console.log(USERSIX);

let usersixcomp = USERSIX.filter(item => item.completed);
// console.log(usersixcomp);
// filter for tasks with a title less than 24 characters long
let shortTitle = data.filter(item => item.title.length < 24);
